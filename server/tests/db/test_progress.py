...
async def test_upsert_progress_update(
    ...
    assert updated["peak_difficulty"] == 30
...
async def test_get_progress_summary(
    ...
    assert row["seen"] == 1  # c2 defaults to peak_difficulty=10, not mastered
...
async def test_list_all_progress_detail_includes_unstarted(
    ...
    started = by_id[c1["id"]]
    assert started["peak_difficulty"] == 10
    ...
    assert unstarted["peak_difficulty"] is None
