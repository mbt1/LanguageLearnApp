...
class TestComputeMastery:
    def test_all_criteria_met(self) -> None:
        assert compute_mastery(
            current_difficulty=ExerciseType.typing,
    ...
    def test_wrong_difficulty_not_mastered(self) -> None:
        assert compute_mastery(
            current_difficulty=ExerciseType.reverse_typing,
            fsrs_stability=MASTERY_STABILITY_THRESHOLD,
            fsrs_state="review",
        ) is False
    ...
    def test_non_typing_difficulties_not_mastered(self, difficulty: ExerciseType) -> None:
        assert compute_mastery(
